...
//Here we test only one slave with address 2 as a demo on the function of WOR, because when more than one slave is applied they are
// all not easier to be waked up,and the master struct might need changing. Added by Icing 2010.8.9
#define RTS_ID 0          // The Unique ID of this RFBee.
byte First_RFBee=2;        // The first RFBee ID in our network. 
byte Last_RFBee=2;       // The Maximum amount of Slaves in this Network
char versionblurb[100] = "v.3 - Reset Commands - MASTER";
static byte current_RFBee;

#define Tx_Reset_Limit 1000     // After 100 Txs the RFBee should be reset (will change later). 
#define Tx_Reboot_Limit 100000  // After 10000 Txs the RFBee should have its EEProm wiped (will change later).
...
void loop(){
  ...
  if(tx_send) {
    ...
    //Transmit 2 more times, so that the slave is much easier to be waked up.Add by Icing
    delay(5);
    transmitData(&serialData[0],len,Config.get(CONFIG_MY_ADDR),current_RFBee);  //Config.get(CONFIG_DEST_ADDR));//transmit
    delay(5);
    transmitData(&serialData[0],len,Config.get(CONFIG_MY_ADDR),current_RFBee);  //Config.get(CONFIG_DEST_ADDR));//transmit
 ...
 /* // The Tx Listen to Rxs Code.
  ...
  if(current_RFBee > Last_RFBee) {      // when reach the max address, return to 2
    ...
    if(tx_send)
      Serial.println(" Tx ==>");
    delay(100);//Shorten the interval  of scaning all slaves.Added by Icing
  }
  delay(10);// Give some time to each slave, otherwise the slave will receive the wrong data for unkown reseaon.Added by Icing
